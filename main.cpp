...
int main() {
    ...
    int move;
    int moveCountX = 0, moveCountO = 0; // Separate move counters
    ...
    while (true) {
        ...
        if (makeMove(board, move, currentPlayer)) {
            if (currentPlayer == 'X') moveCountX++; // Increment respective move counter
            else moveCountO++;
            ...
            if (checkWin(board, currentPlayer)) {
                int winningMoves = (currentPlayer == 'X') ? moveCountX : moveCountO;
                cout << "Congratulations, " << currentName << "! You win in " << winningMoves << " moves!\n";
                break;
